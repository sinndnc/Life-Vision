        ...
        ScrollViewReader{ proxy in
            ScrollView(.horizontal,showsIndicators: false){
                HStack{
                    ForEach(1...days,id: \.self){ day in
                        ...
                        let textColor : Color = viewModel.currentDay.number != day ? .gray : .white
                        let background : Color? = viewModel.currentDay.number != day ? nil : .blue
            ...
            .onAppear{
                if (viewModel.headerViewUIState == .initial){
                    let day = viewModel.currentDay.number
